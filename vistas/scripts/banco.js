...
function listar(){
	tabla=$('#tbllistado').dataTable({
		...
		"ajax":
		{
			url:'../controlador/banco.php?op=listar',
			type: "get",
			dataType : "json",
			error:function(e){
...
function mostrar(idBanco){
	...
	$.ajax({
		...
		url:'../controlador/banco.php?op=mostrar',
		type:"post",
		beforeSend:function(){},
		success: function(response){
			...
			data=$.parseJSON(response);
				$("#idBancoUpdate").val(data['idBanco']);
				$("#nombreUpdate").val(data['nombre']);
				$("#tipoBancoUpdate").val(data['tipoBanco']);
		}
	})
}
...
function guardar(){
	nombre = $('#nombre').val();
	tipoBanco = $('#tipoBanco').val();
	parametros = {"nombre":nombre,"tipoBanco":tipoBanco}
	
	$.ajax({
		data: parametros,
		url: '../controlador/banco.php?op=guardar',
		type: "POST",
		beforeSend:function(){},
		success:function(response){
			if(response == "success"){
...
function Actualizar(){
	idBanco=$('#idBancoUpdate').val();
	nombre=$('#nombreUpdate').val();
	tipoBanco=$('#tipoBancoUpdate').empty();
	parametros ={"idBanco":idBanco,"nombre":nombre,"tipoBanco":tipoBanco}
	$.ajax({
		data: parametros,
		url: '../controlador/banco.php?op=editar',
		type: "POST",
		beforeSend: function(){},
		success:function(response){
...
function confirmarEliminacion(idBanco) {
...
function desactivar(idBanco) {
	$.ajax({
		data: { "idBanco": idBanco },
		url: '../controlador/banco.php?op=desactivar',
		type: "POST",
		beforeSend: function () { }
...
//funcion para desactivar
function confirmarActivacion(idBanco) {
...
function activar(idBanco) {
	$.ajax({
		data: { "idBanco": idBanco },
		url: '../controlador/banco.php?op=activar',
		type: "POST",
		beforeSend: function () { }
	});
}
